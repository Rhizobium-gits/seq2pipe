...
ANALYSIS_REGISTRY: list[AnalysisSpec] = [
    ...
    # ── v3 Additional Analyses (28 new) ──────────────────────────────────

    # Composition (7 new)
    AnalysisSpec(key="phylum_heatmap", phase="composition",
        title="Phylum-Level Abundance Heatmap",
        description="Heatmap of phylum relative abundances per sample, clustered by group. Use seaborn clustermap.",
        requires=["feature_table", "taxonomy"]),
    AnalysisSpec(key="taxa_barplot_top20", phase="composition",
        title="Top 20 Most Abundant Genera (Grouped Bar)",
        description="Grouped bar chart of top 20 genera mean abundance per group. Error bars = SEM.",
        requires=["feature_table", "taxonomy"], min_groups=2),
    AnalysisSpec(key="relative_abundance_area", phase="composition",
        title="Relative Abundance Area Plot (per Sample)",
        description="Stacked area plot of genus relative abundance across samples sorted by group.",
        requires=["feature_table", "taxonomy"]),
    AnalysisSpec(key="taxa_bubble_chart", phase="composition",
        title="Taxa Bubble Chart (Abundance vs Prevalence)",
        description="Scatter/bubble plot: x=mean abundance, y=prevalence (% samples), size=variance. Color by phylum.",
        requires=["feature_table", "taxonomy"]),
    AnalysisSpec(key="firmicutes_bacteroidetes_ratio", phase="composition",
        title="Firmicutes/Bacteroidetes Ratio by Group",
        description="Box plot of F/B ratio per sample, grouped. Add statistical test annotation.",
        requires=["feature_table", "taxonomy"], min_groups=2),
    AnalysisSpec(key="diversity_profile", phase="composition",
        title="Diversity Profile (Hill Numbers q=0,1,2)",
        description="Line plot of Hill diversity numbers (q=0=richness, q=1=Shannon exp, q=2=Simpson inverse) per group.",
        requires=["feature_table"], min_groups=2),
    AnalysisSpec(key="species_accumulation", phase="composition",
        title="Species Accumulation Curve",
        description="Rarefaction-style species accumulation curve per group (number of observed ASVs vs number of samples).",
        requires=["feature_table"], min_groups=2),

    # Alpha (3 new)
    AnalysisSpec(key="alpha_pairwise", phase="alpha",
        title="Pairwise Alpha Diversity Comparison",
        description="For each alpha metric, pairwise Mann-Whitney U between all group pairs with Bonferroni correction. Heatmap of p-values.",
        requires=["alpha"], min_groups=3),
    AnalysisSpec(key="alpha_correlation", phase="alpha",
        title="Alpha Diversity vs Metadata Correlation",
        description="Scatter plot of Shannon diversity vs numeric metadata columns (e.g., age, BMI). Spearman correlation + p-value.",
        requires=["alpha"]),
    AnalysisSpec(key="evenness_dominance", phase="alpha",
        title="Evenness vs Dominance Plot",
        description="Scatter plot: x=Pielou evenness, y=Simpson dominance, colored by group. Shows community structure.",
        requires=["alpha"], min_groups=2),

    # Beta (5 new)
    AnalysisSpec(key="beta_pairwise_heatmap", phase="beta",
        title="Pairwise PERMANOVA Heatmap",
        description="Heatmap of pairwise PERMANOVA p-values between all groups. Annotate with pseudo-F values.",
        requires=["beta"], min_groups=3),
    AnalysisSpec(key="pcoa_3d", phase="beta",
        title="3D PCoA Visualization",
        description="3D scatter plot of first 3 PCoA axes, colored by group. Multiple viewing angles.",
        requires=["beta"], min_groups=2),
    AnalysisSpec(key="distance_boxplot", phase="beta",
        title="Within vs Between Group Distance Boxplot",
        description="Boxplot comparing within-group and between-group distances for each beta metric.",
        requires=["beta"], min_groups=2),
    AnalysisSpec(key="beta_significance_summary", phase="beta",
        title="Beta Diversity Significance Summary",
        description="Table figure: rows=metrics, columns=test/statistic/p/effect. All beta tests in one view.",
        requires=["beta"], min_groups=2),
    AnalysisSpec(key="procrustes", phase="beta",
        title="Procrustes Analysis (Two Beta Metrics)",
        description="Procrustes plot comparing PCoA ordinations from two different beta metrics. Shows congruence.",
        requires=["beta"], min_groups=2),

    # Differential (4 new)
    AnalysisSpec(key="log2fc_heatmap", phase="differential",
        title="Log2 Fold Change Heatmap",
        description="Heatmap of log2 fold changes for top 30 differential genera across group pairs. Clustered.",
        requires=["feature_table", "taxonomy"], min_groups=2),
    AnalysisSpec(key="abundance_stripplot", phase="differential",
        title="Differential Genera Strip Plot",
        description="Strip/swarm plot of top 10 differential genera abundance per group. Individual samples visible.",
        requires=["feature_table", "taxonomy"], min_groups=2),
    AnalysisSpec(key="waterfall_plot", phase="differential",
        title="Waterfall Plot (Sorted Fold Changes)",
        description="Waterfall/bar plot of genera sorted by fold change. Color by enriched/depleted direction.",
        requires=["feature_table", "taxonomy"], min_groups=2),
    AnalysisSpec(key="cladogram", phase="differential",
        title="Taxonomic Cladogram with Differential Highlights",
        description="Circular cladogram showing taxonomic tree with differential genera highlighted by color.",
        requires=["feature_table", "taxonomy"], min_groups=2,
        extra_packages=["ete3"]),

    # Advanced (6 new)
    AnalysisSpec(key="mantel_test", phase="advanced",
        title="Mantel Test (Beta Diversity vs Metadata Distance)",
        description="Mantel test correlating Bray-Curtis distance matrix with metadata distance. Report r and p.",
        requires=["beta"], min_groups=2),
    AnalysisSpec(key="sourcetracker_style", phase="advanced",
        title="Source Proportion Estimate (SourceTracker-style)",
        description="Pie/stacked bar showing estimated source contributions per sample (based on Bayesian mixing model approximation).",
        requires=["feature_table", "taxonomy"], min_groups=3),
    AnalysisSpec(key="volatility_plot", phase="advanced",
        title="Volatility Plot (Longitudinal)",
        description="Line plot tracking diversity metrics and key taxa abundance over time per subject.",
        requires=["feature_table", "alpha"], needs_timepoint=True),
    AnalysisSpec(key="paired_diff_plot", phase="advanced",
        title="Paired Difference Plot",
        description="Spaghetti/slope plot showing per-subject change in diversity between conditions.",
        requires=["alpha"], needs_paired=True, min_groups=2),
    AnalysisSpec(key="adonis_r2_barplot", phase="advanced",
        title="ADONIS R² Variance Decomposition",
        description="Bar plot showing R² (variance explained) by each metadata factor from ADONIS/PERMANOVA.",
        requires=["beta"], min_groups=2),
    AnalysisSpec(key="random_forest_importance", phase="advanced",
        title="Random Forest Feature Importance",
        description="Bar plot of top 20 most important genera from Random Forest classifier. Include OOB error.",
        requires=["feature_table", "taxonomy"], min_groups=2,
        extra_packages=["scikit-learn"]),

    # Publication (3 new)
    AnalysisSpec(key="composite_diversity", phase="publication",
        title="Diversity Composite (Alpha + Beta + Composition)",
        description="6-panel figure: Shannon boxplot, PCoA, genus barplot, rarefaction, PERMANOVA table, volcano.",
        requires=["feature_table", "alpha", "beta"], min_groups=2),
    AnalysisSpec(key="graphical_abstract", phase="publication",
        title="Graphical Abstract",
        description="Single-panel summary figure: study design icon → key result → biological interpretation. Minimalist style.",
        requires=["feature_table"]),
    AnalysisSpec(key="methods_flowchart", phase="publication",
        title="Methods Flowchart",
        description="Flowchart showing analysis pipeline: FASTQ → DADA2 → diversity → differential → interpretation.",
        requires=["feature_table"]),
]
