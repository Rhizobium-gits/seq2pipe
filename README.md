...
- **2 つのモード**: Pipeline (`--auto`) / **Smart (`--smart`)**

### モード比較

| | Pipeline (`--auto`) | **Smart (`--smart`)** |
|---|---|---|
| **用途** | FASTQ → レポートまで全自動 | **データに応じて最適な解析を自動選択** |
| **判断エンジン** | なし（全 71 ステップ実行） | **ベイズ推論 + 学習機構** |
| **判断精度** | 53.8% | **86.1%** |
| **Wrong skips** | 0 | **3 / 21,000,000** (学習後) |
| **判断時間** | — | **60ms** (LLM 不要) |
| **データ圧縮** | 1x | **FT 90x / DM 8x / LLM tokens 82%↓** |
| **71 解析から選択** | 全て実行 | **データに基づき自動選択** |
| **安全モード** | — | `--safe` で偽陰性 = 0 保証 |
| **学習機構** | — | **wrong skip を記録 → 次回は防止** |

### コード生成精度比較 (4 公開データセット × 2 タスク = 8 回)

| 手法 | 成功率 | 平均時間 | 特徴 |
|---|---|---|---|
| Naive LLM (3 retry) | **12%** (1/8) | 77s | 同じエラーを繰り返す |
| AdaptiveCoder (学習+fallback) | **100%** (8/8) | 82s | エラー分析→自動修正→テンプレート |
| テンプレートのみ | **100%** (8/8) | **0.1s** | LLM 不要、770 倍高速 |

> AdaptiveCoder は LLM にエラーの根本原因と具体的修正方法をフィードバックし、自動パッチでコードを直接修正する。全リトライ失敗時はテンプレートフォールバックで 100% を保証。

```bash
# Pipeline: FASTQ から全自動
./launch.sh --fastq-dir ~/data --auto

# Smart: ベイズ推論で最適解析を選択（推奨）
./launch.sh --fastq-dir ~/data --smart \
  --metadata meta.tsv --research-question "Your question"

# Smart + 安全モード: 偽陰性 = 0 保証
./launch.sh --fastq-dir ~/data --smart --safe \
  --metadata meta.tsv --research-question "Your question"
```
...
| 耐障害性 | LLM エラーで停止 | LLM なしでも判断木で完走可能（コード生成除く） |

#### 評価結果（252 独立判断 + 頑健性検証）

| 指標 | 値 | 注記 |
|---|---|---|
| **独立判断数** | **252** (12 シナリオ × 21 ステップ) | これが実質的な評価規模 |
| **ベースライン精度** | 33% (21 中 7 ステップは常に正解) | 判断不要ステップを含む |
| **精度（taxonomy あり）** | **95%** (+31 pp) | 14 の判断対象中 13-14 正解 |
| **精度（taxonomy なし）** | **52%** (改善なし) | データ不完全で判断不能 |
| **全体平均精度** | **73.0%** | taxonomy 有無の混合 |
| **偽陰性スキップ** | **0** | ただし構造的にゼロ（skip のみで add しない） |
| **再現性** | 120 回で std = 0 | 確定的コードなので自明 |
| **重み独立性** | 10,221 設定で同一判断 | 重みは判断に影響しない |
| **scipy 有意性一致** | 99-100% | |
| **Caporaso 2011 結論一致** | 7/7 | |
| **LLM コード生成（registry）** | 47% | |
| **LLM コード生成（investigation）** | 25% | |

> **正直な評価**: 精度 73% は 33% のベースラインを含む。taxonomy ありのデータでは実質 93-100% の判断精度だが、taxonomy なしでは改善ゼロ。偽陰性 0 は「skip しかしない」保守的設計の結果であり、precision の指標としては misleading。エキスパートプロトコルと DecisionEngine は同じ文献ルールを符号化しており、一致は独立検証ではない。
...
**大規模検証: 2,102,520 判断 (12 実データ × 10 回 + 100,000 合成)**

| | Static | v2 (if 文) | v3 (Bayes) |
|---|---|---|---|
| **実データ精度 (12 scenarios)** | 57.5% | 73.0% | **88.5%** |
| **合成データ精度 (100K scenarios)** | 53.7% | 70.4% | **85.8%** |
| **Wrong skips (実データ)** | — | **0** | 30 |
| **Wrong skips (合成)** | — | **0** | 8,691 |
| **合成: v3 が勝つシナリオ** | — | — | **72,966** (73%) |
| **合成: v2 が勝つシナリオ** | — | — | 1,715 (2%) |

> **精度と安全性のトレードオフ:** v3 Bayes は v2 より精度が +15pp 高い（85.8% vs 70.4%）が、false negative（必要な解析の見逃し）が 0.41% 発生する。v2 は false negative = 0 を構造的に保証するが精度は低い。研究のフェーズに応じて選択すべき: 探索段階は v3 (Lite)、最終確認は v2 (AI-Driven)。
