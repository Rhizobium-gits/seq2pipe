...
| 指標 | 値 |
|---|---|
| **判断精度（エキスパート一致率）** | 最大 95%（データ完全時）、平均 66.7% |
| **固定パイプライン精度** | 平均 60.5% |
| **改善幅** | +6.1 pp（最大 +33 pp） |
| **偽陰性スキップ（必要な解析の見逃し）** | **0 / 1,470 判断（0.00%）** |
...
> **注意**: 判断（何をスキップ・追加するか）は 100% LLM 不要・確定的。コード生成（実際に図を描くこと）は LLM 必須で成功率 41%。この 2 つは独立に評価できる。
