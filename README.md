...
- **4 つの操作モード**: Pipeline (`--auto`) / Chat (`--chat`) / AI-Driven (`--ai-driven`) / **Lite (`--lite`)**

### モード別性能比較

| | Pipeline | Chat | AI-Driven | **Lite** |
|---|---|---|---|---|
| **コマンド** | `--auto` | `--chat` | `--ai-driven` | `--lite` |
| **判断精度** | 57.5% | ユーザー依存 | 73.0% | **88.5%** |
| **判断エンジン** | なし（全実行） | 人間 | if 文ルール | **ベイズ推論** |
| **判断時間** | 0s | 対話 | 0.11s | 0.11s |
| **判断に LLM** | 不要 | 不要 | 不要 | **不要** |
| **コード生成に LLM** | 必要 | 必要 | 必要 (47%) | 必要 (47%) |
| **FT 圧縮** | 1x | 1x | 1x | **90x** |
| **距離行列圧縮** | 1x | 1x | 1x | **8x** |
...
| **Wrong skips** | 0 | — | 0 | **0** |
| **GPU** | 不要 | 不要 | 不要 | **不要** |
| **推奨用途** | 初回の包括解析 | 探索的解析 | 論文用の適応解析 | **大規模・低リソース** |
