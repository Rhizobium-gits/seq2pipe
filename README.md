...
#### 評価結果（4 公開データセット × 7 シナリオ × 10 回 = 1,470 判断）

| 指標 | 値 |
|---|---|
| **データセット** | 4 公開データ（Moving Pictures, Atacama, Gneiss, FMT） |
| **シナリオ数** | 12（データセット × 複数グループ列） |
| **実行回数** | 120 回（12 シナリオ × 10 回） |
| **総判断数（実データ）** | **2,520** (12 シナリオ × 21 ステップ × 10 回) |
| **総判断数（合成データ）** | **210,000** (10,000 ランダムシナリオ × 21 ステップ) |
| **総判断数（重み不変性実証）** | **2,520,000** (10,000 重み × 12 シナリオ × 21 ステップ、6.4h 実測) |
| **判断精度（エキスパート一致率）** | 最大 95%、平均 **73.0%** |
| **固定パイプライン精度** | 平均 57.5% |
| **改善幅** | 平均 **+15.5 pp**（最大 **+47.6 pp**） |
| **偽陰性スキップ（必要な解析の見逃し）** | **0 / 2,732,520 判断（0.0000%）** |
| **改善率** | 75.5% のシナリオで改善、24.5% 同等、**0% 悪化** |
| **再現性** | 120 回実行で判断・スコアとも完全同一（std = 0） |
| **重み独立性** | 219 通りの重み設定で全て同一判断（重みは判断に影響しない） |
| **scipy との有意性一致率** | Mann-Whitney 99%、Kruskal-Wallis 100% |
| **Caporaso 2011 の結論との一致** | 7/7（100%） |
| **LLM コード生成成功率（レジストリ）** | 47%（9/19） |
| **LLM コード生成成功率（Investigation）** | 25%（2/8） |

> **重要な発見**: 判断（skip/add）は 100% LLM 不要・確定的であり、8軸スコアの重みにも依存しない。DecisionEngine は生の p 値と効果量から直接判断する。重みは合成スコア表示にのみ影響する。

> **最大効果**: シグナルが欠如するシナリオ（性別、被験者）で最大 +47.6 pp の改善。不要な 10 ステップ以上を正確にスキップ。
