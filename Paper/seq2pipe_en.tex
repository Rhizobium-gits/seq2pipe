...
11 specialized tools to interact with the filesystem,
QIIME2 (auto-detects conda environment; falls back to Docker),
...
\begin{figure}[h]
  ...
  [qiime2_agent.py] <-----> Ollama (localhost:11434)
    ...
    +---> [Tool  6] run_command        (QIIME2: auto-detects conda env / Docker fallback)
    ...
    +---> [Tool 11] log_analysis_step  (manually log QIIME2 steps to ANALYSIS_LOG)
  \end{verbatim}
...
\begin{table}[h]
  ...
  \begin{tabular}{lp{8cm}}
    ...
    \texttt{run\_command} &
      ...
      Automatically detects and prepends the QIIME2 conda environment bin
      (\texttt{miniforge3/envs/qiime2/bin} etc.) to PATH, enabling QIIME2
      execution without Docker. Supports autonomous mode via
      \texttt{SEQ2PIPE\_AUTO\_YES=1} \\
    \texttt{check\_system} &
    ...
    \texttt{log\_analysis\_step} &
      Manually registers a QIIME2 step executed via \texttt{run\_command}
      into \texttt{ANALYSIS\_LOG}, enabling \texttt{build\_report\_tex}
