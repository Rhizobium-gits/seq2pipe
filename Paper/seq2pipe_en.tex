...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small v1.4}
\fancyhead[C]{\small seq2pipe Technical Report}
...
% Title block
\begin{center}
  {\LARGE \textbf{seq2pipe: A Local LLM-Powered}}\\[4pt]
  {\LARGE \textbf{AI Agent for Automated QIIME2 Pipeline Generation}}\\[8pt]
  {\normalsize Technical Report --- Architecture, Implementation, and Evaluation}\\[12pt]
  {\normalsize Rhizobium-gits \quad Claude (Anthropic)}\\[4pt]
  {\small seq2pipe project}\\[2pt]
  {\small \url{https://github.com/Rhizobium-gits/seq2pipe}}\\[8pt]
  {\small March 2026 --- v1.4 (16 modules, 17,000+ lines, 29+43 analysis registry + decision engine)}
\end{center}
...
In v1.2, a \textbf{research-driven autonomous analysis mode} (\texttt{--manual-auto})
was introduced. Given a research question and metadata file, the system
deterministically parses the experimental design (group structure, longitudinal timepoints,
paired subjects) and selects from a registry of 43 analysis specifications
spanning 7 phases: data quality, taxonomic composition, alpha/beta diversity,
differential abundance, advanced network/ordination analyses, and publication-ready
composite figures. Statistical tests are automatically adapted to the experimental
design --- Mann-Whitney~U for unpaired 2-group, Wilcoxon signed-rank for paired,
Kruskal-Wallis with Dunn's post-hoc for multi-group, and PERMANOVA for
beta diversity --- with Benjamini-Hochberg FDR correction applied throughout.
Visualizations include t-SNE, UMAP, Raincloud plots, Volcano plots, LEfSe-style
effect size plots, UpSet diagrams, Ternary plots, Alluvial/Sankey diagrams, and more.
In v1.4, an \textbf{AI-driven adaptive analysis mode} (\texttt{--ai-driven}) was added.
A \texttt{DataInspector} directly parses exported TSV files (feature tables, alpha/beta diversity,
taxonomy) and executes Mann-Whitney~U, Kruskal-Wallis, and PERMANOVA tests in pure Python
without scipy dependency. Results are scored across 8 axes (statistical significance,
biological relevance, novelty, actionability, etc.) and compressed via power-iteration SVD
into 3 principal components. A literature-based \texttt{DecisionEngine} deterministically decides
next analysis steps: Anderson~(2001) for PERMANOVA$\rightarrow$betadisper logic,
Willis~(2019) for effect-size-driven alpha interpretation,
Gloor~et~al.~(2017) for compositional bias detection.
The LLM role is restricted to creative investigation hypotheses;
Python decisions always take priority via \texttt{merge\_with\_llm()},
and the system runs to completion even under LLM failure.
...
select between four operating modes (directed analysis, autonomous, interactive chat,
and research-driven manual-auto).
...
\subsection{Research-Driven Autonomous Analysis (\texttt{--manual-auto})}

Added in v1.2, \texttt{manual\_auto\_agent.py} (1,262 lines) provides a
research-question-driven analysis mode that automatically builds and executes
a comprehensive analysis plan.

\paragraph{Experimental Design Parsing}
\texttt{parse\_metadata()} deterministically analyzes the metadata TSV and produces
an \texttt{ExperimentalDesign} dataclass:
\begin{itemize}[nosep]
  \item \textbf{Group column}: auto-selected from priority list (treatment, group, condition, genotype, diet, etc.)
  \item \textbf{Timepoint}: column name patterns (timepoint, day, week, visit, dpi, etc.)
  \item \textbf{Subject column}: column name patterns (subject, patient, donor, mouse, etc.)
  \item \textbf{Paired design}: detected via subject$\times$group cross-check
  \item \textbf{Imbalanced design}: warning if max/min group size ratio $> 2.0$
  \item \textbf{Continuous variables}: numeric columns with $> 10$ unique values
\end{itemize}

\paragraph{43-Analysis Registry}
\texttt{ANALYSIS\_REGISTRY} defines 43 analysis specifications across 7 phases:
\begin{enumerate}[nosep]
  \item \textbf{Data Quality} (3): DADA2 stats, read depth, ASV frequency
  \item \textbf{Composition} (8): phylum/genus/family stacked bars, heatmap, violin,
    indicator species, etc.
  \item \textbf{Alpha Diversity} (5): multi-metric boxplot, raincloud plot,
    rarefaction, effect size forest plot, longitudinal trajectory
  \item \textbf{Beta Diversity} (9): PCoA (all metrics + 95\% ellipses), NMDS,
    t-SNE, UMAP, PCA-CLR biplot, PERMANOVA/ANOSIM, beta dispersion, dendrogram
  \item \textbf{Differential Abundance} (5): volcano plot (FDR), MA plot,
    forest plot (Cliff's delta), LEfSe-style, multi-group KW heatmap
  \item \textbf{Advanced} (10): co-occurrence network, correlation clustermap,
    ternary plot (3 groups), UpSet diagram, alluvial/Sankey, Venn diagram, etc.
  \item \textbf{Publication} (3): 4-panel main figure, 6-panel supplementary,
    statistical summary table
\end{enumerate}

Each specification encodes required data categories, min/max group counts, and
longitudinal/paired requirements. \texttt{build\_analysis\_plan()} filters the registry
based on data availability and experimental design.

\paragraph{Adaptive Statistical Test Selection}
\texttt{\_select\_stat\_test()} chooses the optimal test based on design:
\begin{itemize}[nosep]
  \item 2 groups, unpaired $\to$ Mann-Whitney U test
  \item 2 groups, paired $\to$ Wilcoxon signed-rank test
  \item 3+ groups, unpaired $\to$ Kruskal-Wallis + Dunn's post-hoc (Bonferroni)
  \item 3+ groups, paired $\to$ Friedman test + Nemenyi post-hoc
  \item Beta diversity $\to$ PERMANOVA (999 permutations)
  \item Multiple testing $\to$ Benjamini-Hochberg FDR correction
\end{itemize}

\paragraph{Execution Flow}
\texttt{run\_manual\_auto()} iterates over plan steps, for each:
(1) builds a step-specific prompt with research context, design summary, and prior results;
(2) requests code from the LLM;
(3) executes with up to 3 retries (including auto pip install);
(4) records results and passes context to subsequent steps.
...
\subsection{AI-Driven Adaptive Analysis (\texttt{--ai-driven})}

Added in v1.4, \texttt{ai\_driven\_agent.py} and \texttt{eval\_mediator.py} implement
a three-layer architecture that evaluates analysis results from raw data and
deterministically decides next steps via a literature-based decision tree.

\paragraph{Layer 1: DataInspector}
The \texttt{DataInspector} class directly parses QIIME2 exported TSV files:
\begin{itemize}
  \item \textbf{Feature table}: ASV $\times$ sample matrix $\rightarrow$ sparsity, read depth CV, Good's coverage
  \item \textbf{Alpha diversity}: per-metric TSV $\rightarrow$ group-split $\rightarrow$ Mann-Whitney~U (2 groups) / Kruskal-Wallis (3+ groups) with Cliff's delta / $\eta^2$ effect sizes
  \item \textbf{Beta diversity}: distance matrix TSV $\rightarrow$ within/between-group distance separation $\rightarrow$ pseudo-PERMANOVA (199 permutations)
  \item \textbf{Taxonomy}: genus-level relative abundances per group $\rightarrow$ fold change, absolute difference, Bray-Curtis-like composition shift
\end{itemize}
All tests are implemented in pure Python (\texttt{\_mann\_whitney\_u()}, \texttt{\_kruskal\_wallis()},
normal CDF approximation via Abramowitz \& Stegun, Wilson-Hilferty $\chi^2$ approximation),
eliminating the scipy dependency.

\paragraph{Layer 2: EvalMediator + DecisionEngine}
Each analysis step is scored across 8 axes:
statistical\_significance (log-transformed $p$-values, 20\%),
biological\_relevance (fold change + known taxa hits, 18\%),
actionability (downstream trigger count, 15\%),
novelty (dissimilarity from prior findings, 12\%),
data\_quality (sparsity, CV, 10\%),
effect\_magnitude (Cliff's $d$ / pseudo-$F$, 10\%),
confidence (minimum group $n$, 8\%),
reproducibility (cross-category consistency, 7\%).
The score matrix ($n_{\text{steps}} \times 8$) is compressed to 3 principal components
via power-iteration SVD on the covariance matrix.

The \texttt{DecisionEngine} implements deterministic decision trees:
\begin{itemize}
  \item Anderson (2001): PERMANOVA significant $\rightarrow$ betadisper mandatory; not significant $\rightarrow$ skip all beta follow-ups
  \item Willis (2019): effect size $> 0.5$ $\rightarrow$ deep investigation; $\leq 0.5$ $\rightarrow$ rarefaction to check sampling artifact
  \item Gloor et al.\ (2017): fold change $> 50$ at low abundance ($< 1\%$) $\rightarrow$ compositional bias warning + filtered retest
  \item McMurdie \& Holmes (2014): read depth CV $> 0.5$ $\rightarrow$ rarefaction mandatory
\end{itemize}

\paragraph{Layer 3: LLM Auxiliary}
The LLM role is restricted to creative investigation hypotheses that the decision tree
cannot generate. \texttt{Decision.merge\_with\_llm()} ensures Python decisions always
override LLM decisions. On LLM failure, the \texttt{except} block applies Python-only
decisions to the plan queue, enabling analysis completion without any LLM involvement.
...
\begin{itemize}
  ...
  \item Four operation modes: directed (Mode 1), autonomous (Mode 2),
    interactive chat (Mode 3), and research-driven manual-auto (Mode 4)
  \item \textbf{Research-driven autonomous analysis (\texttt{manual\_auto\_agent.py})}:
    deterministically parses experimental design from metadata and selects from
    a 43-analysis registry with adaptive statistical test selection
  \item \textbf{43-analysis registry}: PCoA, NMDS, t-SNE, UMAP, raincloud plots,
    volcano plots, LEfSe-style, UpSet diagrams, ternary plots, alluvial/Sankey,
    PERMANOVA, forest plots, and more
...
\begin{itemize}
  ...
  \item Machine learning-based analyses (Random Forest / XGBoost feature importance,
    supervised classification) in the deterministic module
  \item LLM pass-2 in manual-auto mode for research-question-specific analysis steps
  \item Multi-factor experimental designs (two or more interacting factors)
\end{itemize}
...
The system combines four complementary module layers:
...
and \texttt{manual\_auto\_agent.py} selects from a 43-analysis registry
to execute research-question-driven analyses adapted to the experimental design.
The \texttt{--manual-auto} mode introduced in v1.2 enables researchers to specify
their research question and metadata, after which the system deterministically
parses the experimental design and executes a comprehensive suite of analyses ---
including volcano plots, LEfSe-style effect sizes, PERMANOVA, t-SNE, UMAP,
raincloud plots, and 30+ additional visualization types --- with statistical tests
automatically selected based on design (Mann-Whitney / Wilcoxon / Kruskal-Wallis /
Friedman / PERMANOVA) and FDR multiple testing correction applied throughout.
...
from raw FASTQ data through publication-quality figures
and reports --- entirely offline and without any cloud dependencies.
