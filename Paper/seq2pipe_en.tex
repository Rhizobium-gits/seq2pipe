...
Typing ``report'' generates an HTML report; typing ``PDF'' compiles a
LaTeX document using \texttt{lualatex} or \texttt{xelatex}.
...
\subsection{Automatic JPEG Conversion for Figures}
...
immediately after code execution and converts them to JPEG using the macOS
...
the extension ``MUST be \texttt{.jpg}'', reducing the frequency of incorrect
...
Within the refinement session, keyword detection triggers report generation:
...
This replaces the previous \texttt{tectonic}-based approach with
standard TeX Live / MacTeX tooling, broadening compatibility.
...
and after execution, and PDF/SVG files are immediately converted to JPEG.
...
natural-language figure refinement and report generation
...
into a report automatically by \texttt{generate\_html\_report()} or
\texttt{generate\_latex\_report()} (\texttt{report\_generator.py}).
The LLM generates a Japanese caption for each figure and an overall summary;
the LaTeX variant is compiled with \texttt{lualatex}.
...
\begin{itemize}
  ...
  \item \textbf{Post-analysis refinement mode (\texttt{run\_refinement\_loop})}:
    iterative figure refinement via natural language; loop exits on empty Enter
  \item \textbf{Automatic PDF/SVG to JPEG conversion (\texttt{sips})}:
    no extra dependencies on macOS; prevents ``unsupported format'' errors in Preview
  \item \textbf{Metadata-free diversity analysis}:
    individual \texttt{qiime diversity alpha/beta} commands as fallback
    when \texttt{core-metrics-phylogenetic} cannot run without metadata
  \item \textbf{HTML and LaTeX/PDF report auto-generation} (\texttt{report\_generator.py}):
    type ``report'' for HTML, ``PDF'' for \texttt{lualatex}/\texttt{xelatex} PDF;
    LLM generates Japanese figure captions and summary
...
\begin{itemize}
  \item A Streamlit browser GUI (\texttt{app.py}) has been implemented as an optional interface; the terminal CLI remains the primary workflow
  \item Support for additional marker genes (ITS, 18S rRNA)
  \item Pillow-based PDF/SVG $\to$ JPEG conversion for Linux and Windows
  ...
  \item Automatic launch of the refinement loop after \texttt{--auto} mode completion
\end{itemize}
...
LaTeX/PDF reports compiled with \texttt{lualatex} or \texttt{xelatex}.
...
and a formatted PDF report --- entirely offline and without any cloud dependencies.
