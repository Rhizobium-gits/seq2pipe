...
that generates \textbf{15 publication-quality PNG figures without any LLM dependency}.
...
In autonomous mode (\texttt{--auto}), the system completes all analysis in 3 steps:
STEP~1 (QIIME2 pipeline) $\to$ STEP~2 (deterministic analysis, 15 figures) $\to$
STEP~3 (HTML report).
...
comprehensive analysis module reliably produces 15 publication-quality figures,
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Guaranteed deterministic analysis}:
    ...
    reliably generates 15 figures regardless of model quality.
\end{enumerate}
...
In \texttt{--auto} mode, a 3-step automated pipeline executes:
STEP~1 runs \texttt{pipeline\_runner.py} for the QIIME2 pipeline and exports results,
STEP~2 runs \texttt{analysis.py} to deterministically generate 15 PNG figures,
...
\begin{figure}[h]
  ...
  \end{verbatim}
  \caption{Three-step architecture of seq2pipe}
  \label{fig:arch}
\end{figure}
...
generate 15 publication-quality PNG figures (Table~\ref{tab:analysis_figures}).
...
\begin{table}[h]
  \centering
  \caption{15 figures generated by \texttt{analysis.py}}
...
15 core figures are always generated regardless of LLM performance.
...
Following QIIME2 analysis, the \texttt{--auto} mode executes a 3-step
automated pipeline:

\begin{enumerate}[leftmargin=2em]
  \item \textbf{STEP 1}: QIIME2 pipeline (8 steps above) + result export
  \item \textbf{STEP 2}: Deterministic comprehensive analysis (\texttt{analysis.py})
    $\to$ \textbf{15 PNG figures}:
    \begin{itemize}
      \item fig01: DADA2 denoising statistics
      \item fig02: Sequencing depth per sample
      \item fig03: Alpha diversity (Shannon / Faith PD / Observed ASVs)
      \item fig04: Shannon diversity per sample
      \item fig05--08: PCoA (Bray-Curtis / Jaccard / Unweighted UniFrac / Weighted UniFrac)
      \item fig09: Beta diversity distance heatmaps (2$\times$2)
      \item fig10: Top 30 ASV heatmap
      \item fig11: Alpha diversity correlations
      \item fig12: ASV richness vs depth
      \item fig13: Genus-level composition (with classifier)
      \item fig14: Phylum-level composition (with classifier)
      \item fig15: Genus-level heatmap (with classifier)
    \end{itemize}
...
does not use the LLM, so all 15 figures are generated identically
...
STEP~2 generated 15 PNG figures via \texttt{analysis.py}, and
...
All 15 figures and the HTML report were generated automatically by
...
\begin{itemize}
  ...
  \item \textbf{Deterministic comprehensive analysis module (\texttt{analysis.py})}:
    15 publication-quality PNG figures generated reliably without LLM dependency
  \item \textbf{SILVA 138 classifier auto-discovery (\texttt{--classifier})}:
    automatic genus/phylum composition figure generation
  \item \textbf{3-step automated pipeline}:
    STEP~1 (QIIME2) $\to$ STEP~2 (deterministic analysis) $\to$ STEP~3 (HTML report)
...
\begin{itemize}
  ...
  \item Adding rarefaction curves, NMDS, and other analyses to \texttt{analysis.py}
\end{itemize}
...
generates 15 publication-quality PNG figures without LLM dependency,
...
from raw FASTQ data through 15 publication-quality figures
