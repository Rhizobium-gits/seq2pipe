...
修正セッション中に「レポート」と入力すると HTML レポート、
「PDF」と入力すると \texttt{lualatex} / \texttt{xelatex} による LaTeX/PDF レポートを自動生成する。
...
\begin{figure}[h]
  ...
  [report_generator.py]  HTML / LaTeX+PDF レポート生成
    +-- generate_html_report()   base64 埋め込み HTML レポート
    `-- generate_latex_report()  lualatex / xelatex で PDF をコンパイル
  \end{verbatim}
  \caption{seq2pipe のアーキテクチャ}
  \label{fig:arch}
\end{figure}
...
\texttt{sips} コマンドで自動的に JPEG へ変換し、元ファイルを削除する。
これにより macOS プレビュー等で問題なく図を確認できる。
また、プロンプトに「拡張子は必ず \texttt{.jpg}」という指示を追加し、
...
修正セッション中に特定のキーワードが入力されるとレポートが生成される:

\begin{itemize}
  \item 「レポート」・\texttt{html} など → \texttt{generate\_html\_report()}:
    図を base64 で埋め込んだ自己完結型 HTML ファイルを生成する。
    LLM が各図の日本語解釈文と総合サマリーを生成し、
    解析パラメータ表・パイプラインステップを合わせて記録する。
  \item 「PDF」・\texttt{latex} など → \texttt{generate\_latex\_report()}:
...
従来 \texttt{tectonic} に依存していたレポート生成を
macOS/Linux 標準の LaTeX ツールチェーン（MacTeX）で置き換えたことにより、
追加インストールなしで PDF レポートを生成できる環境が拡大した。
...
全フェーズ完了後、振り返り・修正モードが起動し、
自然言語指示による図の修正と
\texttt{report\_generator.py} による HTML / LaTeX+PDF レポート出力が行える。
...
\texttt{\_run\_pipeline.py} を用いた単一コマンドで
QIIME2（DADA2 デノイジング + core-metrics-phylogenetic）を実行し、
エクスポートデータを \texttt{chat\_agent.InteractiveSession} へ渡して
ダウンストリーム可視化を行った。
6 種類の図が自動生成され、PDF レポートにまとめられた。
図はリポジトリの \texttt{Figure/} ディレクトリに格納されている。

\subsection{α 多様性}

4 つの α 多様性指標（Shannon エントロピー、Faith's PD、Pielou's evenness、
Observed features）をサンプルごとに計算し、
ストリッププロットを重ねた 2$\times$2 パネルで表示した（図~\ref{fig:alpha}）。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.92\textwidth]{../Figure/alpha_diversity.png}
  \caption{ヒト便検体 10 サンプルの α 多様性。
    各パネルは DADA2 デノイジング済みフィーチャーテーブルから算出した
    異なる指標を示す。ストリッププロットが各サンプルの実測値を重ねて表示している。}
  \label{fig:alpha}
\end{figure}

\subsection{β 多様性}

3 種類の非類似度指標で β 多様性を評価した。
Bray--Curtis PCoA（図~\ref{fig:bc}）と
Unweighted UniFrac PCoA（図~\ref{fig:unifrac}）は
scikit-learn MDS（\texttt{n\_components=2, dissimilarity=`precomputed'}）で計算し、
Jaccard 距離はクラスタリング付きヒートマップで可視化した（図~\ref{fig:jaccard}）。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\textwidth]{../Figure/bray_curtis_pcoa.png}
  \caption{Bray--Curtis PCoA。サンプルは tab10 パレットでサンプル ID ごとに
    色分けし、プロット上に直接ラベルを付した。MDS 座標は
    QIIME2 エクスポートされた距離行列から計算した。}
  \label{fig:bc}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \caption{Unweighted UniFrac PCoA。QIIME2 の core-metrics-phylogenetic
    パイプラインが計算した系統的距離を 2 次元 MDS で射影した。}
  \label{fig:unifrac}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \caption{Ward 連結法による階層クラスタリング付き Jaccard 距離ヒートマップ
    （seaborn \texttt{clustermap}）。両軸が 10 サンプルを示し、
    色の濃さがペアワイズ非類似度を表す。}
  \label{fig:jaccard}
\end{figure}
...
図~\ref{fig:dada2} に示す。
グループ棒グラフは \texttt{denoising\_stats/stats.tsv} から
人手を一切介さずに生成した。
...
\begin{figure}[htbp]
  ...
  \caption{10 便検体の DADA2 デノイジング統計。
    各グループが 1 サンプルに対応し、棒の色が 5 つの処理ステージを区別する。
    フィルタリング効率がサンプル間で概ね均一であることが示された。}
  \label{fig:dada2}
\end{figure}

\subsection{Shannon 多様性バイオリンプロット}

図~\ref{fig:shannon} は Shannon 多様性の水平バイオリンプロットで、
サンプルを Shannon 値の昇順に並べ、全体平均を縦線で示している。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\textwidth]{../Figure/shannon_violin.png}
  \caption{サンプルごとの Shannon α 多様性を水平バイオリンプロットで表示。
    サンプルは Shannon 値の昇順に並べており、縦線は全サンプルの平均値を示す。}
  \label{fig:shannon}
\end{figure}

6 つの図と解釈文は \texttt{InteractiveSession.generate\_report()} によって
日本語 PDF レポート（\texttt{report\_ja.pdf}）に自動統合された。
LLM が各図のキャプションを生成し、\texttt{tectonic} でコンパイルした。
...
\begin{itemize}
  ...
  \item \textbf{小型 LLM 向けロバストネス機能}: 4 層フォールバック機構
    ...
    ステップ 6 の 1 ショットフォールバック・繰り返し検出）により、
    7B モデルでも確実に図を生成
  \item 2 つの操作モード: 指定解析（モード 1）・完全自律（モード 2 / \texttt{--auto}）
  \item \textbf{解析後の振り返り・修正モード（\texttt{run\_refinement\_loop}）}:
    自然言語指示で図を対話的に修正・再実行
  \item \textbf{PDF/SVG → JPEG 自動変換（\texttt{sips}）}:
    macOS 内蔵コマンドで追加依存なし、プレビューで開けない PDF 図を自動 JPEG 変換
  ...
  \item \textbf{HTML / LaTeX+PDF レポート自動生成}（\texttt{report\_generator.py}）:
    「レポート」→ HTML、「PDF」→ \texttt{lualatex}/\texttt{xelatex} による PDF
...
\begin{itemize}
  ...
  \item PDF レポートのコンパイルには \texttt{lualatex} または \texttt{xelatex}
    （MacTeX / TeX Live）のインストールが必要である。インストールされていない環境では
    \texttt{.tex} ファイルのみが保存される。
  \item \texttt{qiime2\_agent.py}（QIIME2 パイプライン生成）には read-first パターンが未適用であり、
    \texttt{code\_agent.py} との統合が今後の課題である。
  \item PDF/SVG の自動 JPEG 変換は macOS の \texttt{sips} に依存しており、
...
\begin{itemize}
  \item Streamlit ブラウザ GUI（\texttt{app.py}）は任意利用可として実装済み。今後は主要インターフェースをターミナル CLI に統一しつつ UI の拡充を図る
  \item ITS / 18S rRNA など他のマーカー遺伝子への対応
  \item \texttt{Pillow} を利用した Linux / Windows 向け PDF/SVG → JPEG 変換の実装
  \item 差次存在量解析（volcano plot）・機械学習（Random Forest）の
    自律タスクへの追加
  \item 統計的検定結果の図への自動アノテーション
  \item \texttt{--auto} モード終了後の振り返りループの自動起動
\end{itemize}
...
PDF レポートまでを、完全オフラインかつクラウド不要の環境で自動化できる。
