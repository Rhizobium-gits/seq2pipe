...
STEP 1（QIIME2 パイプライン）$\to$ STEP 2（決定論的解析・15 図）$\to$ STEP 3（HTML レポート）。
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{STEP 2}: 決定論的包括解析（\texttt{analysis.py}）$\to$ \textbf{15 種類の PNG 図}を生成:
    \begin{itemize}
...
\begin{itemize}
  ...
  \item \textbf{3 ステップ自動化パイプライン}:
    STEP 1（QIIME2）$\to$ STEP 2（決定論的解析）$\to$ STEP 3（HTML レポート）
