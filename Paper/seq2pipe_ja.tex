...
と通信し、11 種類のツールを介してファイルシステム・QIIME2（conda 環境を自動検出、fallback で Docker）・
Python 解析エンジンと連携する。
...
\begin{figure}[h]
  ...
  [qiime2_agent.py] <-----> Ollama (localhost:11434)
    ...
    +---> [Tool  6] run_command        (QIIME2 実行: conda env 自動検出 / Docker fallback)
...
\begin{table}[h]
  ...
  \begin{tabular}{lp{7.5cm}}
    ...
    \texttt{run\_command} &
      ...
      QIIME2 conda 環境（\texttt{miniforge3/envs/qiime2} 等）を自動検出し
      PATH に追加するため、Docker なしでも QIIME2 コマンドが実行可能。
      \texttt{SEQ2PIPE\_AUTO\_YES=1} で確認をスキップする自律モードに対応 \\
    \texttt{check\_system} &
    ...
    \texttt{log\_analysis\_step} &
      \texttt{run\_command} 経由の QIIME2 操作を \texttt{ANALYSIS\_LOG} に手動登録する \\
    \bottomrule
  \end{tabular}
\end{table}
