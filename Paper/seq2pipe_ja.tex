...
LLM に依存せず \textbf{15 種類の出版品質 PNG 図を確実に生成} する。
...
自律モード（\texttt{--auto}）では 3 ステップで全解析を完了する:
STEP 1（QIIME2 パイプライン）→ STEP 2（決定論的解析・15 図）→ STEP 3（HTML レポート）。
...
決定論的な包括解析モジュールによる 15 種類の図の確実な生成、
LLM による柔軟な追加解析、
...
\begin{figure}[h]
  ...
  \end{verbatim}
  \caption{seq2pipe の 3 ステップアーキテクチャ}
  \label{fig:arch}
\end{figure}
...
15 種類の出版品質 PNG 図を生成する（表~\ref{tab:analysis_figures}）。

このモジュールは LLM の応答品質に左右されないため、
どのモデルを使用しても同一の解析結果が得られる。
SILVA 138 分類器による分類学的解析結果が利用可能な場合は
fig13〜fig15（属・門レベルの組成図）も自動的に生成される。

\begin{table}[h]
  \centering
  \caption{\texttt{analysis.py} が生成する 15 種類の図}
  ...
  \begin{tabular}{clp{6cm}}
    ...
    fig09 & β 多様性距離ヒートマップ (2$\times$2) & seaborn \\
...
LLM の性能に関わらず 15 種類の基本図は常に生成される。
...
QIIME2 解析後、\texttt{--auto} モードでは以下の 3 ステップ自動化パイプラインが実行される:

\begin{enumerate}[leftmargin=2em]
  \item \textbf{STEP 1}: QIIME2 パイプライン（上記 8 ステップ）+ 結果エクスポート
  \item \textbf{STEP 2}: 決定論的包括解析（\texttt{analysis.py}）→ \textbf{15 種類の PNG 図}を生成:
    \begin{itemize}
      \item fig01: DADA2 デノイジング統計
      \item fig02: シーケンシング深度
      \item fig03: α 多様性（Shannon / Faith PD / Observed ASVs）
      \item fig04: Shannon 多様性（サンプル別）
      \item fig05--08: PCoA（Bray-Curtis / Jaccard / Unweighted UniFrac / Weighted UniFrac）
      \item fig09: β 多様性距離ヒートマップ（2$\times$2）
...
なお、\texttt{analysis.py} による決定論的解析（STEP 2）は
LLM を使用しないため、モデルの選択に関わらず同一の 15 図が生成される。
...
STEP 2 で \texttt{analysis.py} が 15 種類の PNG 図を生成し、
...
3 つの α 多様性指標（Shannon エントロピー、Faith's PD、Observed ASVs）を
...
\begin{figure}[htbp]
  ...
  \caption{サンプルごとの Shannon α 多様性ストリッププロット。}
  \label{fig:shannon}
\end{figure}

\subsection{β 多様性}

4 種類の非類似度指標で β 多様性を評価した。
...
\begin{figure}[htbp]
  ...
  \caption{β 多様性距離ヒートマップ（2$\times$2）。
...
全 15 図と HTML レポートは \texttt{analysis.py} と
...
\begin{itemize}
  ...
  \item \textbf{決定論的包括解析モジュール（\texttt{analysis.py}）}:
    LLM に依存せず 15 種類の出版品質 PNG 図を確実に生成
  \item \textbf{SILVA 138 分類器の自動探索（\texttt{--classifier}）}:
    分類学的解析と属・門レベル組成図の自動生成
  \item \textbf{3 ステップ自動化パイプライン}:
    STEP 1（QIIME2）→ STEP 2（決定論的解析）→ STEP 3（HTML レポート）
  ...
  \item \textbf{解析後の振り返り・修正モード}:
    自然言語指示で図を対話的に修正・再実行
  \item \textbf{PDF/SVG → PNG 自動変換（\texttt{sips}）}:
    macOS 内蔵コマンドで追加依存なし
...
\begin{itemize}
  ...
  \item \texttt{Pillow} を利用した Linux / Windows 向け PDF/SVG → PNG 変換の実装
  \item 差次存在量解析（volcano plot）・機械学習（Random Forest）の
    自律タスクへの追加
  \item 統計的検定結果の図への自動アノテーション
  \item \texttt{analysis.py} への追加解析（レアファクション曲線、NMDS 等）の統合
\end{itemize}
...
\texttt{analysis.py} が LLM に依存せず 15 種類の出版品質 PNG 図を確実に生成し、
...
これにより研究者は生 FASTQ データから 15 種類の出版品質の図と
