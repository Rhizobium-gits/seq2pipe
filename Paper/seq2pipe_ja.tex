...
% タイトル
\begin{center}
  {\LARGE \textbf{seq2pipe: ローカル LLM による}}\\[4pt]
  {\LARGE \textbf{QIIME2 パイプライン自動生成 AI エージェント}}\\[8pt]
  {\normalsize Technical Report --- 設計・実装・評価}\\[12pt]
  ...
  {\small 2026 年 3 月 --- v1.4 (16 モジュール, 17,000+ 行, 29+43 解析レジストリ + 判断木エンジン)}
\end{center}
...
v1.2 では \textbf{研究目的駆動の自律解析モード}（\texttt{--manual-auto}）を追加した。
ユーザーが研究の問いとメタデータを指定すると、実験デザイン（群数・縦断・ペア設計）を
決定論的に解析し、43 種の解析レジストリから条件に合う 25〜41 ステップを自動選択して実行する。
統計検定は Mann-Whitney U / Wilcoxon / Kruskal-Wallis / Friedman / PERMANOVA から
デザインに基づいて自動選択され、Benjamini-Hochberg FDR 補正が適用される。
可視化は PCoA・NMDS に加え t-SNE・UMAP・Raincloud plot・Volcano plot・LEfSe-style・
UpSet diagram・Ternary plot・Alluvial/Sankey など 40 種以上に対応する。
v1.4 では \textbf{AI 駆動の適応的解析モード}（\texttt{--ai-driven}）を追加した。
DataInspector が feature table・alpha/beta diversity TSV・taxonomy を直接パースし、
Mann-Whitney U / Kruskal-Wallis / PERMANOVA を scipy に依存せず純 Python で実行する。
8 軸スコア評価（統計的有意性・生物学的関連性・新規性・行動可能性等）の結果は
SVD べき乗法で 3 主成分に圧縮され、文献ベースの確定的判断木（DecisionEngine）が
次の解析ステップを自動決定する。LLM の役割は創造的調査仮説の生成に限定され、
LLM 障害時も Python 判断木のみで解析を完走できる。
...
4 つの操作モード（指定解析 / 自律エージェント / 対話チャット / 研究目的駆動）を選択させる。
...
\subsection{研究目的駆動の自律解析モード（\texttt{--manual-auto}）}

v1.2 で追加された \texttt{manual\_auto\_agent.py}（1,262 行）は、
メタデータと研究の問いから包括的な解析プランを自動構築・実行する。

\paragraph{実験デザイン解析}
\texttt{parse\_metadata()} はメタデータ TSV を決定論的に解析し、
\texttt{ExperimentalDesign} データクラスを生成する:
\begin{itemize}[nosep]
  \item \textbf{グループ列}: 優先リスト（treatment, group, condition, genotype, diet 等）から自動選択
  \item \textbf{タイムポイント}: カラム名パターン（timepoint, day, week, visit, dpi 等）
  \item \textbf{被験者列}: カラム名パターン（subject, patient, donor, mouse 等）
  \item \textbf{ペアデザイン}: 被験者列$\times$グループ列の組み合わせから自動判定
  \item \textbf{不均衡検出}: 群間サンプル数比 $> 2.0$ で警告
  \item \textbf{連続変数}: 数値型かつユニーク値 $> 10$ のカラム
\end{itemize}

\paragraph{43 種の解析レジストリ}
\texttt{ANALYSIS\_REGISTRY} は 7 フェーズ・43 種の解析仕様を定義する:
\begin{enumerate}[nosep]
  \item \textbf{Data Quality}（3 種）: DADA2 統計、リード深度、ASV 頻度分布
  \item \textbf{Composition}（8 種）: 門/属/科積み上げ棒、ヒートマップ、バイオリン、
    Indicator Species 等
  \item \textbf{Alpha Diversity}（5 種）: マルチメトリクス箱ひげ図、Raincloud plot、
    レアファクション、効果量 Forest plot、縦断軌跡
  \item \textbf{Beta Diversity}（9 種）: PCoA（全メトリクス+95\%信頼楕円）、NMDS、
    t-SNE、UMAP、PCA-CLR biplot、PERMANOVA/ANOSIM、Beta Dispersion、樹形図
  \item \textbf{Differential Abundance}（5 種）: Volcano plot（FDR 補正）、MA plot、
    Forest plot（Cliff's delta）、LEfSe-style、多群 KW ヒートマップ
  \item \textbf{Advanced}（10 種）: 共起ネットワーク、相関 clustermap、Ternary plot（3群）、
    UpSet diagram、Alluvial/Sankey、Venn diagram 等
  \item \textbf{Publication}（3 種）: 4パネル Main Figure、6パネル Supplementary、統計サマリーテーブル
\end{enumerate}

各解析仕様は必要データカテゴリ・最小/最大グループ数・縦断/ペア要件を持ち、
\texttt{build\_analysis\_plan()} がデータ可用性と実験デザインに基づいて自動フィルタリングする。

\paragraph{データ適応型統計検定}
\texttt{\_select\_stat\_test()} が実験デザインに基づいて最適な検定を自動選択する:
\begin{itemize}[nosep]
  \item 2 群・対応なし $\to$ Mann-Whitney U test
  \item 2 群・対応あり $\to$ Wilcoxon signed-rank test
  \item 3+ 群・対応なし $\to$ Kruskal-Wallis + Dunn's post-hoc（Bonferroni）
  \item 3+ 群・対応あり $\to$ Friedman test + Nemenyi post-hoc
  \item $\beta$ 多様性 $\to$ PERMANOVA（999 permutations）
  \item 多重検定補正 $\to$ Benjamini-Hochberg FDR
\end{itemize}

\paragraph{実行フロー}
\texttt{run\_manual\_auto()} は各ステップについて:
(1) ステップ固有のプロンプトを構築（研究目的・デザイン・前ステップ結果を含む）、
(2) LLM にコード生成を依頼、
(3) 実行・リトライ（最大 3 回、pip 自動インストール含む）、
(4) 結果を記録し次ステップへコンテキストとして渡す。

\subsection{AI 駆動の適応的解析モード（\texttt{--ai-driven}）}

v1.4 で追加された \texttt{ai\_driven\_agent.py} および \texttt{eval\_mediator.py} は、
解析結果を実データから定量評価し、文献ベースの判断木で次のステップを確定的に決定する
3 層アーキテクチャを実装する。

\paragraph{Layer 1: DataInspector（実データ読み込み層）}
\texttt{DataInspector} クラスは QIIME2 のエクスポート済み TSV ファイルを直接パースする。
\begin{itemize}
  \item \textbf{Feature table}: ASV $\times$ サンプル行列 → スパース度、リード深度 CV、Good's coverage
  \item \textbf{Alpha diversity}: 各メトリクス TSV → 群別に分割 → 自前の Mann-Whitney U（2 群）/ Kruskal-Wallis（3+ 群）検定。Cliff's delta / $\eta^2$ 近似による効果量も計算
  \item \textbf{Beta diversity}: 距離行列 TSV → 群内/群間距離を分離 → 擬似 PERMANOVA（199 permutation）
  \item \textbf{Taxonomy}: 属レベル相対豊度を群別に集計 → fold change、絶対差、組成シフト（Bray-Curtis 風距離）
\end{itemize}
すべての統計検定は \texttt{scipy} に依存せず純 Python で実装されており（\texttt{\_mann\_whitney\_u()}, \texttt{\_kruskal\_wallis()}, 正規近似 CDF, Wilson-Hilferty $\chi^2$ 近似）、QIIME2 conda 環境外でも動作する。

\paragraph{Layer 2: EvalMediator + DecisionEngine（評価・判断層）}
各解析ステップの結果を 8 軸で定量評価する:
statistical\_significance（p 値対数変換, 20\%）、
biological\_relevance（fold change + 既知菌属ヒット, 18\%）、
actionability（次ステップトリガー, 15\%）、
novelty（既知発見との非類似度, 12\%）、
data\_quality（スパース度・CV, 10\%）、
effect\_magnitude（Cliff's delta / pseudo-F, 10\%）、
confidence（群あたり最小 $n$, 8\%）、
reproducibility（同カテゴリ間一貫性, 7\%）。
スコア行列（$n_{\mathrm{steps}} \times 8$）は共分散行列のべき乗法で 3 主成分に圧縮される。

\texttt{DecisionEngine} は文献ベースの確定的判断木を実装する:
\begin{itemize}
  \item Anderson (2001): PERMANOVA 有意 $\rightarrow$ betadisper 必須; 非有意 $\rightarrow$ beta 系全ステップをスキップ
  \item Willis (2019): effect size $> 0.5$ $\rightarrow$ 深掘り; $\leq 0.5$ $\rightarrow$ rarefaction で sampling artifact を確認
  \item Gloor et al.\ (2017): fold change $> 50$ かつ低豊度（$< 1\%$）$\rightarrow$ 組成バイアス警告 + フィルタ後再検定
  \item McMurdie \& Holmes (2014): read depth CV $> 0.5$ $\rightarrow$ rarefaction 必須
\end{itemize}

\paragraph{Layer 3: LLM 補助判断}
LLM の役割は Python 判断木が生成できない創造的調査仮説（例: 「379$\times$ の Escherichia enrichment は contamination か real bloom か」）に限定される。
\texttt{Decision.merge\_with\_llm()} により Python 判断が常に LLM 判断に優先し、
LLM が \texttt{skip} したものを Python が \texttt{add} していれば Python が勝つ。
LLM 障害時は \texttt{except} ブロック内で Python 判断のみを \texttt{plan\_queue} に適用し、解析を完走する。
...
\begin{itemize}
  ...
  \item 4 つの操作モード: 指定解析（モード 1）・完全自律（モード 2）・対話チャット（モード 3）・研究目的駆動（モード 4 / \texttt{--manual-auto}）
  \item \textbf{研究目的駆動の自律解析（\texttt{manual\_auto\_agent.py}）}:
    メタデータから実験デザインを決定論的に解析し、43 種の解析レジストリから
    条件に合うステップを自動選択・実行。統計検定はデザインに基づいて自動選択
  \item \textbf{43 種の解析レジストリ}: PCoA・NMDS・t-SNE・UMAP・Raincloud plot・
    Volcano plot・LEfSe-style・UpSet diagram・Ternary plot・Alluvial/Sankey・
    PERMANOVA・Forest plot 等を網羅
...
\begin{itemize}
  ...
  \item 機械学習（Random Forest / XGBoost）による分類器構築と特徴量重要度の自動解析
  \item manual-auto モードの LLM パス 2 による研究目的固有の解析ステップ追加
  \item 多因子デザイン（2 因子以上の交互作用）への対応
\end{itemize}
...
本システムは 4 つの相補的なモジュール層を組み合わせる:
\texttt{qiime2\_agent.py} が 11 ツールで QIIME2 パイプライン生成を担い、
\texttt{analysis.py} が LLM に依存せず 29 種類の出版品質 PNG 図を確実に生成し、
\texttt{code\_agent.py} が vibe-local 方式の 5 ツールで追加の柔軟な解析を提供し、
\texttt{manual\_auto\_agent.py} が 43 種の解析レジストリから
実験デザインに合う解析を自動選択・実行する。
v1.2 で追加された \texttt{--manual-auto} モードにより、
研究者は研究の問いとメタデータを指定するだけで、
実験デザインに最適化された包括的な解析（Volcano plot・LEfSe-style・
PERMANOVA・t-SNE・UMAP・Raincloud plot 等 40 種以上）を全自動で実行できる。
統計検定は実験デザイン（群数・ペア・縦断）に基づいて自動選択され、
FDR 多重検定補正が適用される。
解析完了後は振り返り・修正モードにより自然言語で図を対話的に改善でき、
\texttt{report\_generator.py} が HTML または LaTeX+PDF レポートを自動生成する。
これにより研究者は生 FASTQ データから出版品質の図とレポートまでを、
完全オフラインかつクラウド不要の環境で自動化できる。
