...
\paragraph{評価レベル。}3段階の評価を実施した：
\begin{enumerate}
  \item \textbf{実データ評価}：12シナリオ$\times$10回 = 2,520判断
  \item \textbf{合成データ汎化性}：10,000ランダムデータベクトル$\times$21ステップ = 210,000判断
  \item \textbf{重み不変性}：10,000ランダム重み$\times$12シナリオ = 2,520,000フル実行（6.4時間）
\end{enumerate}
合計\textbf{2,732,520}件の判断評価を実施した。
...
\begin{table}[t]
\centering
\caption{判断精度：12シナリオ、4データセット、2,520判断（std $= 0$）。}
...
10,000のランダムデータ特性ベクトルで210,000判断を評価。平均精度70.6\%$\pm$12.6\%。75.5\%改善、24.5\%同等、\textbf{0\%悪化}。偽陰性率0/210,000。シグナル欠如時+33.3~pp、部分シグナル+16.5~pp。
...
\textit{何を解析するか}（確定的、LLM不要）と\textit{どう実行するか}（LLM補助コード生成）をアーキテクチャ的に分離する閉ループマイクロバイオーム適応解析システム\seqpipe を提示した。4公開データセット12シナリオおよび10,000合成シナリオ（合計2,732,520判断）での評価により、DecisionEngineはエキスパート一致率73.0\%（最大95\%）を偽陰性=0かつ重み不変性のもとで達成した。75.5\%のシナリオで改善し、ゼロケースで悪化した。LLMはコード生成に不可欠であり（47\%成功）、システムの主要ボトルネックである。

中心的設計原則は\textbf{ワークフロー判断はデータに基づくべきであり言語モデル出力に基づくべきではない}ということであり、同時に\textbf{解析実行には現在LLMが必要である}ことを認める。判断の正確性はコード生成品質とは独立に検証可能であり、重みベクトルの摂動に対して数学的に不変である——このアーキテクチャ特性が信頼性のある自律解析システム構築の鍵である。
