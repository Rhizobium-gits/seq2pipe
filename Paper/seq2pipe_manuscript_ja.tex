...
ローカル大規模言語モデル（7Bパラメータ）はコード生成と仮説生成の補助として機能するが、LLM完全障害時もシステムは実行を完了する。
...
ローカル7Bモデルが初期計画（失敗時はドメイン知識フォールバック）、Pythonコード（最大3回リトライ）、リプランニング時の調査仮説を生成する。マージ関数$\mathrm{merge}(d_{\mathrm{py}}, d_{\mathrm{llm}})$はPython判断のスキップがLLM提案の追加を上書きすることを保証する。LLM完全障害下では確定的判断のみが適用される。
...
確定的データ解釈・判断と確率的コード生成をアーキテクチャ的に分離する閉ループマイクロバイオーム適応解析システム\seqpipe を提示した。4公開データセット7シナリオでの評価により、DecisionEngineはエキスパート一致率最大95\%を偽陰性スキップ=0かつ絶対的再現性で達成し、7B LLMは定義済みタスクで54\%の成功を示した。

中心的設計原則——解析判断はデータに基づくべきであり言語モデル出力に基づくべきではない——は、監査可能で再現可能でLLM障害に対して頑健なシステムをもたらす。確定的推論と確率的生成の分離が、信頼性のある自律解析システム構築のための鍵となるアーキテクチャ原則であると主張する。
